...
for j = 1:length(overlaps)
    ...
    for i = 2 : nn.n   
        ...
        [t1s, t2s] = cf_analysis({nn1.W{1}, nn2.W{1}, Snn2.W{1}}, X);   % I think this one carries out classification analysis
        t1sum(k,:) = t1s;   %t1s has dimensions (3,1) and it's the accuracy on the T1 images over the 3 phases of training: on T1 images, on T2 images and sleep
        t2sum(k,:) = t2s;   %same but for the accuracy on T2 images
...
plot_weights_hist_new({nn1.W{1}, nn2.W{1}, Snn2.W{1}}, X)   % this takes the last trial (trials are only f)
