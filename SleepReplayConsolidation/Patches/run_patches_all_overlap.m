...
for j = 1:length(overlaps)
    [X,y] = create_permutations(width, overlaps(j), num_images);    %create the patches dataset

    nn = nnsetup([100 4]);      %setup the Feedforward Backpropagating NN (nnsetup(layers))
    for i = 2 : nn.n   %nn.n = # of layers
        % Initialize Weights - choose between [-0.1 0.1]
        nn.W{i - 1} = (rand(nn.size(i), nn.size(i - 1)) - 0.5) * 0.01 * 2;
        nn.vW{i - 1} = zeros(size(nn.W{i-1}));  %weights vector
    end
        ...
        if j == 14  % plot only for the case of 14 overlapping pixels (such as it's done in the paper)
